...
class NTXentLoss(torch.nn.Module):
    ...
    def __init__(self, device, batch_size, temperature, use_cosine_similarity ,feature_dimension , num_nodes):
        ...
        self.weight1 = nn.Parameter(torch.randn(512, feature_dimension * num_nodes)).to(self.device)
        self.weight2 = nn.Parameter(torch.randn(feature_dimension * num_nodes, 512)).to(self.device)
        self.bias1 = nn.Parameter(torch.randn(512)).to(self.device)
        self.bias2 = nn.Parameter(torch.randn(feature_dimension * num_nodes)).to(self.device)
    ...
    def forward(self, zis, zjs):
        ...
        zjs = F.linear(zjs, self.weight1, self.bias1)
        zjs = F.linear(zjs, self.weight2, self.bias2)
